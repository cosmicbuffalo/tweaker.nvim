...
function M.generate(name)
    ...
    local base = vim.g.colors_name or "(unknown)"
    add(string.format("-- Generated by tweaker.nvim from the %q colorscheme.", base))
    add("--")
    add("-- Overwriting: :TweakerBake won't clobber an existing file unless you pass")
    add('-- the ! bang. The default output name is "<active colorscheme>-tweaked", so')
    add(string.format("-- :TweakerBake! regenerates THIS file only while %q is your active", base))
    add("-- colorscheme; under a different colorscheme it writes a differently-named file.")
